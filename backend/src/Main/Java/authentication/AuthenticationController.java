...
public class AuthenticationController {
    ...
    public Response authenticateUser(UserLogin login)
    {
        try {
            ...
            Account account = authenticate(login.getUsername(), login.getPassword(), login.getSignInCode());
    ...
    private Account authenticate(String username, String password, String signInCode) throws Exception {
        Account account = accountRepository.getUser(username, password);
        if(account != null){
            if(account.getSignInCode().equals(signInCode)) {
                long time = System.currentTimeMillis();
                if(account.getTime() + 300000 >= time) {
                    return account;
                }
            }
            throw new Exception();
